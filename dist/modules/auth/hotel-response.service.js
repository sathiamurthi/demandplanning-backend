...
exports.hotelResponseRouter = void 0;
// ============================================================
...
exports.hotelResponseRouter = (0, express_1.Router)();
// ── GET /v1/public/hotel-response/wa-status  (diagnostic — no secrets exposed)
exports.hotelResponseRouter.get('/wa-status', (_req, res) => {
...
exports.hotelResponseRouter.post('/outreach', async (req, res) => {
    try {
        const { inquiry_id, hotel_name, hotel_email, hotel_phone, city, inquiry_snapshot } = req.body;
        if (!inquiry_id?.trim() || !hotel_name?.trim()) {
            fail(res, 'inquiry_id and hotel_name are required');
            return;
        }
        const [row] = await (0, db_1.query)(`INSERT INTO hotel_outreaches
       ...
       RETURNING id, token, status, created_at`, [
        ...
        if (hotel_phone?.trim()) {
            ...
            const result = await (0, whatsapp_1.sendWhatsAppText)(hotel_phone.trim(), lines);
            wa_sent = result.sent;
...
exports.hotelResponseRouter.get('/outreach/status', async (req, res) => {
    try {
        ...
        const rows = await (0, db_1.query)(`SELECT id, token, inquiry_id, hotel_name, status,
              hotel_action, hotel_quote, hotel_message, hotel_contact_name,
              responded_at, created_at
       FROM hotel_outreaches
       WHERE token::text = ANY(ARRAY[${placeholders}])`, tokens);
        ok(res, rows);
    }
    catch (e) {
        fail(res, e.message);
...
exports.hotelResponseRouter.get('/:token', async (req, res) => {
    try {
        const { token } = req.params;
        const row = await (0, db_1.queryOne)(`SELECT id, token, inquiry_id, inquiry_snapshot, hotel_name, hotel_email, hotel_phone,
              city, status, hotel_action, hotel_quote, hotel_message, hotel_contact_name,
              responded_at, created_at
       FROM hotel_outreaches WHERE token = $1::uuid`, [token]);
        if (!row) {
            fail(res, 'Inquiry not found or link expired', 404);
            return;
        }
        // Auto-advance Sent → Viewed
        if (row.status === 'Sent') {
            await (0, db_1.query)(`UPDATE hotel_outreaches SET status='Viewed', updated_at=NOW() WHERE token=$1::uuid`, [token]);
            row.status = 'Viewed';
        }
        ok(res, row);
    }
...
const RespondSchema = zod_1.z.object({
    contact_name: zod_1.z.string().min(1).max(100),
    action: zod_1.z.enum(['Accept', 'Quote', 'Hold', 'Reject', 'Future']),
    quote_amount: zod_1.z.number().positive().optional(),
    message: zod_1.z.string().max(1000).optional(),
});
exports.hotelResponseRouter.post('/:token/respond', async (req, res) => {
    try {
        const { token } = req.params;
        const body = RespondSchema.parse(req.body);
        const existing = await (0, db_1.queryOne)('SELECT id, status FROM hotel_outreaches WHERE token=$1::uuid', [token]);
        if (!existing) {
            fail(res, 'Inquiry not found or link expired', 404);
            return;
        }
        if (existing.status === 'Responded') {
            fail(res, 'You have already responded to this inquiry', 409);
            return;
        }
        const [row] = await (0, db_1.query)(`UPDATE hotel_outreaches
       ...
       RETURNING id, token, inquiry_id, hotel_name, status, hotel_action, hotel_quote,
                 hotel_message, hotel_contact_name, responded_at`, [body.action, body.quote_amount ?? null, body.message ?? null, body.contact_name, token]);
        ok(res, row);
    }
    catch (e) {
        if (e.message?.includes('invalid input syntax')) {
        ...
        if (e instanceof zod_1.z.ZodError) {
            fail(res, (e.issues?.[0]?.message) || 'Validation error');
            return;
        }
        fail(res, e.message);
    }
