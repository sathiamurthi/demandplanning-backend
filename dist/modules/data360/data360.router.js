...
exports.data360Router.post('/admin/grant-quota', data360Auth, async (req, res) => {
    try {
        if (!isUnlimitedUser(req.d360User.email)) {
            fail(res, 'Forbidden', 403);
            return;
        }
        const { user_email, additional_documents } = req.body;
        if (!user_email?.trim() || !Number.isFinite(additional_documents) || additional_documents <= 0) {
            fail(res, 'user_email and a positive additional_documents are required');
            return;
        }
        const [updated] = await (0, db_1.query)(`UPDATE data360_users SET purchased_document_quota = purchased_document_quota + $1 WHERE email=$2 RETURNING id, email, purchased_document_quota`, [additional_documents, user_email.trim().toLowerCase()]);
        if (!updated) {
            fail(res, 'No Data360 user found with that email', 404);
            return;
        }
        ok(res, updated);
    }
...
function isUnlimitedUser(email) {
    return (email || '').trim().toLowerCase() === DATA360_SUPERADMIN_EMAIL;
}
async function getDocumentUsage(userId) {
    const row = await (0, db_1.queryOne)(`SELECT COALESCE(SUM(total_rows),0)::int AS used FROM data360_batches WHERE user_id=$1`, [userId]);
    return row?.used || 0;
}
async function getDocumentLimit(userId) {
    const row = await (0, db_1.queryOne)(`SELECT purchased_document_quota FROM data360_users WHERE id=$1`, [userId]);
    return FREE_DOCUMENT_LIMIT + (row?.purchased_document_quota || 0);
}
exports.data360Router.get('/quota', data360Auth, async (req, res) => {
    try {
        if (isUnlimitedUser(req.d360User.email)) {
...
exports.data360Router.post('/ai-extract', data360Auth, async (req, res) => {
    try {
        ...
        const aiRes = await (0, aiService_1.callAI)({ cacheablePrompt, prompt, maxTokens: 4096, jsonResponse: true, preferredOrder: (0, aiService_1.costEffectiveOrder)() });
...
exports.data360Router.post('/ai-extract-image', data360Auth, async (req, res) => {
    try {
        ...
        const aiRes = await (0, aiService_1.callAI)({ cacheablePrompt, prompt, imageBase64: image_base64, mimeType: mediaType, maxTokens: 4096, jsonResponse: true, preferredOrder: (0, aiService_1.costEffectiveOrder)() });
        await logAiUsage(req.d360User.sub, batch_label, file_label, aiRes);
        const result = parseExtractionResponse(aiRes.text, fields);
        if (!result) {
            fail(res, `AI returned invalid JSON — please try again. Provider: ${aiRes.provider}. Raw response: ${aiRes.text.slice(0, 300)}`, 502);
...
JSON object only.`;
function parseArbitraryJson(rawText) {
    const cleaned = rawText.replace(/```json|```/g, '').trim();
    try {
...
exports.data360Router.post('/school/generate-chapter-guide', data360Auth, async (req, res) => {
    try {
        const { class_level, board, subject, chapter_name } = req.body;
        if (!class_level?.trim()) {
            fail(res, 'class_level is required');
            return;
        }
        ...
        if (!subject?.trim()) {
            fail(res, 'subject is required');
            return;
        }
        if (!chapter_name?.trim()) {
            fail(res, 'chapter_name is required');
            return;
        }
        ...
        const prompt = buildStudyGuideDynamicPrompt(class_level, board, subject, `Chapter Name: ${chapter_name}\n\nBased on your knowledge of the curriculum, generate the Study Pack JSON.`);
        const aiRes = await (0, aiService_1.callAI)({ cacheablePrompt: SCHOOL_CHAPTER_GENERATE_PROMPT, prompt, maxTokens: 8000, jsonResponse: true, preferredOrder: (0, aiService_1.costEffectiveOrder)() });
