...
exports.runTeaDailyDigestJob = runTeaDailyDigestJob;
exports.runTeaWeeklyGrowerPaymentJob = runTeaWeeklyGrowerPaymentJob;
...
async function claimJobRun(jobKey, tenantId, runKey) {
    const rows = await (0, db_1.query)(`INSERT INTO tea_job_runs (job_key, tenant_id, run_key) VALUES ($1,$2,$3)
     ON CONFLICT (job_key, tenant_id, run_key) DO NOTHING RETURNING 1`, [jobKey, tenantId, runKey]);
    return rows.length > 0;
}
async function activeTeaTenants() {
    return (0, db_1.query)(`SELECT DISTINCT t.id AS tenant_id
     FROM tenants t
...
async function ownerPhone(tenantId) {
    const owner = await (0, db_1.query)(`SELECT phone FROM users WHERE tenant_id=$1 AND role='owner' AND phone IS NOT NULL AND phone<>'' LIMIT 1`, [tenantId]);
...
async function runTeaDailyDigestJob() {
    try {
        const now = istNow();
        if (now.getHours() !== 7)
            return; // only inside the 7am IST hour
        ...
        for (const { tenant_id } of tenants) {
         ...
         FROM tea_grower_settlements WHERE tenant_id=$1 AND paid=FALSE`, [tenant_id]);
            const [pendingDispatch] = await (0, db_1.query)(`SELECT COUNT(*)::int AS count, COALESCE(SUM(total_kg), 0) AS kg
         FROM tea_collection_batches WHERE tenant_id=$1 AND status='pending_dispatch'`, [tenant_id]);
            const overdueMaint = await (0, db_1.query)(`SELECT COUNT(*)::int AS count FROM tea_vehicle_maintenance
         WHERE tenant_id=$1 AND due_date IS NOT NULL AND due_date < CURRENT_DATE`, [tenant_id]);
            ...
            if (lines.length === 1)
                lines.push('- All clear — nothing pending. Have a great day!');
            const result = await (0, whatsapp_1.sendWhatsAppText)((0, whatsapp_1.normalizeWhatsAppPhone)(phone), lines.join('\n'));
            if (!result.sent)
                logger_1.logger.warn(`[TeaJob:dailyDigest] tenant=${tenant_id} not sent: ${result.error || result.skipped}`);
        }
    }
    catch (e) {
        logger_1.logger.error('[TeaJob:dailyDigest] Job failed:', e.message);
    }
}
...
async function runTeaWeeklyGrowerPaymentJob() {
    try {
        const now = istNow();
        if (now.getDay() !== 1 || now.getHours() !== 9)
            return; // Monday, 9am IST hour only
        ...
        for (const { tenant_id } of tenants) {
            ...
            if (!claimed)
                continue;
            const settlements = await (0, db_1.query)(`SELECT s.*, g.name AS grower_name, g.phone AS grower_phone
         FROM tea_grower_settlements s JOIN tea_growers g ON g.id = s.grower_id
         WHERE s.tenant_id=$1 AND s.paid=FALSE AND g.phone IS NOT NULL AND g.phone<>''`, [tenant_id]);
            for (const s of settlements) {
                ...
                const result = await (0, whatsapp_1.sendWhatsAppText)((0, whatsapp_1.normalizeWhatsAppPhone)(s.grower_phone), msg);
                if (!result.sent)
                    logger_1.logger.warn(`[TeaJob:weeklyPayment] tenant=${tenant_id} grower=${s.grower_id} not sent: ${result.error || result.skipped}`);
            }
        }
    }
    catch (e) {
        logger_1.logger.error('[TeaJob:weeklyPayment] Job failed:', e.message);
    }
}
