...
exports.teaAgentAuthRouter = void 0;
// ============================================================
...
exports.teaAgentAuthRouter = (0, express_1.Router)();
...
exports.teaAgentAuthRouter.post('/register', async (req, res) => {
    try {
        const body = RegisterSchema.parse(req.body);
        const slug = body.companyCode.trim().toLowerCase();
        const tenant = await (0, db_1.queryOne)(`SELECT t.id, t.name
       FROM tenants t
       JOIN tenant_industries ti ON ti.tenant_id = t.id
       JOIN industry_configs ic ON ic.id = ti.industry_id
       WHERE t.slug = $1 AND ic.industry_id = 'tea' AND t.is_active = TRUE`, [slug]);
        if (!tenant)
            return fail(res, 'Company code not found — check with your factory for the correct code');
        ...
        const [created] = await (0, db_1.query)(`INSERT INTO users (tenant_id, email, phone, password_hash, role, first_name, last_name, is_active, reg_type)
       ...
       RETURNING id, email, phone, first_name, last_name, created_at`, [
        ...
        const owner = await (0, db_1.queryOne)(`SELECT phone FROM users WHERE tenant_id=$1 AND role='owner' AND phone IS NOT NULL AND phone<>'' LIMIT 1`, [tenant.id]);
        if (owner?.phone) {
            (0, whatsapp_1.sendWhatsAppText)(owner.phone, `🌿 *TeaFactory360*\n\nA new agent signed up pending your approval:\n*${body.firstName} ${body.lastName || ''}*\n${phoneNorm || emailNorm}\n\nApprove them from Team & Roles in the app.`).catch(() => { });
        }
        ok(res, {
            id: created.id,
