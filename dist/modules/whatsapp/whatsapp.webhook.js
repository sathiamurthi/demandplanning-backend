...
exports.waWebhookRouter = void 0;
...
exports.waWebhookRouter = (0, express_1.Router)();
const VERIFY_TOKEN = meta_config_1.metaConfig.verifyToken;
const APP_SECRET = meta_config_1.metaConfig.appSecret;
const footer = meta_config_1.botFooter;
// ── Signature verification ────────────────────────────────────
function verifySignature(rawBody, signature) {
    if (!APP_SECRET)
        return true; // skip in dev if not configured
    const expected = 'sha256=' + crypto_1.default.createHmac('sha256', APP_SECRET).update(rawBody).digest('hex');
    try {
...
exports.waWebhookRouter.get('/', (req, res) => {
...
exports.waWebhookRouter.post('/', async (req, res) => {
    ...
    try {
        ...
        for (const msg of messages) {
            if (msg.type !== 'text') {
                await (0, whatsapp_1.sendWhatsAppText)(msg.from, '👋 I can only understand text messages right now.\nType *help* to see what I can do.');
                continue;
            }
            const waPhone = (0, whatsapp_1.normalizeWhatsAppPhone)(msg.from);
...
async function getSession(waPhone) {
    return (0, db_1.queryOne)(`SELECT ws.*, u.first_name, u.last_name, u.role,
     ...
     WHERE ws.wa_phone = $1`, [waPhone]);
}
async function touchSession(waPhone) {
    await (0, db_1.query)(`UPDATE wa_sessions SET last_seen_at = NOW() WHERE wa_phone = $1`, [waPhone]);
}
// ── Main message handler ──────────────────────────────────────
async function handleMessage(waPhone, text) {
    ...
    if (lower === 'hi' || lower === 'hello' || lower === 'start') {
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, userFlowCard());
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, benefitCard());
        return;
    }
    // ── Help ──
    if (lower === 'help') {
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, helpText());
        return;
    }
    // ── Vendor workflow reply: YES / NO / COMMENT <msg> ──
    // Check before everything else so vendor replies are caught even without a session
    const handledByWorkflow = await (0, workflow_service_1.handleVendorWorkflowReply)(waPhone, text);
    ...
    if (session) {
        ...
        if (lower === 'today' || lower === 'sales today' || lower === 'today sales') {
            await handleTodaySummary(waPhone, session);
            return;
        }
        if (lower === 'low stock' || lower === 'lowstock' || lower === 'reorder') {
            await handleLowStock(waPhone, session);
            return;
        }
        if (lower === 'me' || lower === 'account' || lower === 'whoami') {
            await (0, whatsapp_1.sendWhatsAppText)(waPhone, `👤 *Your Account*\n\n` +
        ...
        if (searchTerm) {
            await handleSearch(waPhone, session, searchTerm);
            return;
        }
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, `🤔 I didn't understand that.\n\nType *help* to see all commands, or just type a product name to search.\n` +
            footer());
        return;
    }
    ...
    await (0, whatsapp_1.sendWhatsAppText)(waPhone, `👋 *Welcome to DemandGenius!*\n\n` +
...
// ── Link handler ──────────────────────────────────────────────
async function handleLink(waPhone, args) {
    const parts = args.split(/\s+/);
    if (parts.length < 2) {
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, `❌ Usage: *link {email} {password}*\n\nExample:\nlink owner@shop.com MyPassword123`);
        return;
    }
    const email = parts[0].toLowerCase();
    const password = parts.slice(1).join(' ');
    const user = await (0, db_1.queryOne)(`SELECT u.*, s.id AS store_id, s.name AS store_name FROM users u
     LEFT JOIN stores s ON s.tenant_id = u.tenant_id AND s.is_active = TRUE
     WHERE u.email = $1 AND u.is_active = TRUE
     LIMIT 1`, [email]);
    if (!user) {
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, `❌ Account not found or inactive. Check your email and try again.`);
        return;
    }
    const valid = await bcryptjs_1.default.compare(password, user.password_hash || '');
    if (!valid) {
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, `❌ Incorrect password. Please try again.`);
        return;
    }
    // Upsert session
    await (0, db_1.query)(`INSERT INTO wa_sessions (wa_phone, user_id, tenant_id, store_id)
     VALUES ($1, $2, $3, $4)
     ON CONFLICT (wa_phone) DO UPDATE
       SET user_id=$2, tenant_id=$3, store_id=$4, last_seen_at=NOW()`, [waPhone, user.id, user.tenant_id, user.store_id]);
    await (0, whatsapp_1.sendWhatsAppText)(waPhone, `✅ *Linked successfully!*\n\nWelcome, *${user.first_name}*! 👋\n\n` +
...
// ── Logout handler ────────────────────────────────────────────
async function handleLogout(waPhone) {
    await (0, db_1.query)(`DELETE FROM wa_sessions WHERE wa_phone = $1`, [waPhone]);
    await (0, whatsapp_1.sendWhatsAppText)(waPhone, `✅ *Unlinked successfully.*\n\nYour number is no longer connected to any store account.\n\n` +
        `To reconnect: *link {email} {password}*\n` +
        footer('You can still search products and browse stores without logging in.'));
}
// ── Search handler ────────────────────────────────────────────
async function handleSearch(waPhone, session, term) {
    const results = await (0, db_1.query)(`SELECT
     ...
     ORDER BY
       ...
       i.name
     LIMIT ${meta_config_1.metaConfig.bot.maxPrivateResults}`, [session.store_id, `%${term}%`, `${term}%`]);
    if (!results.length) {
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, `🔍 No results found for *"${term}"* in your store.\n\nTry a different spelling or shorter keyword.\n` +
            footer('Search products across all stores on our explore page.'));
        return;
    }
    const lines = [`🔍 *${results.length} result${results.length > 1 ? 's' : ''} for "${term}"* — ${session.store_name}\n`];
    for (const item of results) {
        ...
        lines.push(`*${item.name}*${skuStr}${brandStr}${catStr}`, `${stockStr}${priceStr}${locStr}`, '');
    }
    lines.push(footer('View full inventory & place orders on the web.'));
    await (0, whatsapp_1.sendWhatsAppText)(waPhone, lines.join('\n').trimEnd());
}
// ── Low stock handler ─────────────────────────────────────────
async function handleLowStock(waPhone, session) {
    const results = await (0, db_1.query)(`SELECT i.name, i.sku, i.current_stock, i.reorder_level,
     ...
     LIMIT ${meta_config_1.metaConfig.bot.maxLowStockResults}`, [session.store_id]);
    if (!results.length) {
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, `✅ *All items are above reorder level!*\n\nNo low stock alerts for *${session.store_name}* right now.\n` +
            footer('View full inventory analytics on the web.'));
        return;
    }
    const lines = [`⚠️ *Low Stock Alert — ${results.length} item${results.length > 1 ? 's' : ''} in ${session.store_name}*\n`];
    for (const item of results) {
...
async function handleTodaySummary(waPhone, session) {
    ...
    const [summary] = await (0, db_1.query)(`SELECT
     ...
     FROM sales
     WHERE store_id = $1 AND DATE(created_at) = $2`, [session.store_id, today]);
    const rev = Number(summary?.total_revenue || 0);
    const bills = summary?.total_bills || 0;
    const items = summary?.total_items || 0;
    await (0, whatsapp_1.sendWhatsAppText)(waPhone, `📊 *Today's Summary — ${today}*\n` +
...
async function handlePublicSearch(waPhone, term) {
    const results = await (0, db_1.query)(`SELECT i.name, i.brand, i.selling_price,
     ...
     LIMIT ${meta_config_1.metaConfig.bot.maxPublicResults}`, [`%${term}%`, `${term}%`]);
    if (!results.length) {
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, `🔍 No results found for *"${term}"*\n\n` +
    ...
    const lines = [`🔍 *${results.length} result${results.length > 1 ? 's' : ''} for "${term}"*\n`];
    for (const item of results) {
    ...
    lines.push(`📞 _Phone numbers & stock quantities visible after login_`, footer('Login for live stock levels, prices & direct contact.'));
    await (0, whatsapp_1.sendWhatsAppText)(waPhone, lines.join('\n').trimEnd());
}
// ── Public store explorer ─────────────────────────────────────
async function handleExploreStores(waPhone, city) {
    const vals = [];
    let where = `WHERE s.is_active = TRUE AND t.is_active = TRUE AND s.city IS NOT NULL`;
    if (city) {
        where += ` AND s.city ILIKE $1`;
        vals.push(`%${city}%`);
    }
    const stores = await (0, db_1.query)(`SELECT s.name AS store_name, s.city, s.state,
     ...
     ${where} ORDER BY s.name LIMIT ${meta_config_1.metaConfig.bot.maxStoreResults}`, vals);
    if (!stores.length) {
        await (0, whatsapp_1.sendWhatsAppText)(waPhone, `🏪 No stores found${city ? ` in *${city}*` : ''}.\n\n` +
            `Try: *stores bangalore*  •  *stores nilgiris*  •  *stores chennai*\n` +
            footer('See all registered stores on our explore page.'));
        return;
    }
    const lines = [`🏪 *${stores.length} store${stores.length > 1 ? 's' : ''}${city ? ' in ' + city : ' — all cities'}*\n`];
    for (const s of stores) {
        const loc = [s.city, s.state].filter(Boolean).join(', ');
        lines.push(`*${s.store_name}*  [${s.industry}]`, `📍 ${loc}  •  ${s.product_count} products`, `📞 _Contact details available for registered users_`, '');
    }
    lines.push(`💡 *Search products across all stores — just type a name*`, `_e.g: rice, paracetamol, oil filter, amul_`, footer('View store profiles, products & contact info online.'));
    await (0, whatsapp_1.sendWhatsAppText)(waPhone, lines.join('\n').trimEnd());
}
// ── Search term extractor ─────────────────────────────────────
function extractSearchTerm(lower, original) {
    ...
    if (!commands.has(lower) && lower.length >= 2)
        return original.trim();
    return null;
}
// ── User flow card (sent on hi/hello/start) ───────────────────
function userFlowCard() {
    return (`👋 *Welcome to DemandGenius!*\n` +
        `_Your AI-powered life dashboard_\n\n` +
        `━━━━━━━━━━━━━━━━━━━━\n` +
        `📱 *HOW TO USE THE APP*\n` +
        `━━━━━━━━━━━━━━━━━━━━\n\n` +
        `*Step 1 — Open the app*\n` +
        `👉 ${meta_config_1.metaConfig.exploreUrl}\n\n` +
...
// ── Help text ─────────────────────────────────────────────────
function helpText() {
    return (`📦 *DemandGenius — AI Store Assistant*\n` +
