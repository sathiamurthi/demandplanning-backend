...
exports.handleVendorWorkflowReply = handleVendorWorkflowReply;
...
async function createNotification(opts) {
    await (0, db_1.query)(`INSERT INTO notifications (phone, type, title, body, action_type, action_data, ref_type, ref_id)
...
async function notifyVendor(matchId, req, vendor) {
    ...
    const result = await (0, whatsapp_1.sendWhatsAppText)(vendor.phone, lines);
    if (result.sent || result.skipped) {
        await (0, db_1.query)(`UPDATE workflow_vendor_matches SET wa_notified_at = NOW(), status = 'notified' WHERE id = $1`, [matchId]);
    }
    return result;
}
// ── Find matching vendors ─────────────────────────────────────
async function findVendors(vendorType, city, limit = 5) {
    // Stem plural (hotels → hotel)
    const stem = vendorType.toLowerCase().endsWith('s') && vendorType.length > 4
        ? vendorType.slice(0, -1) : vendorType;
    const rows = await (0, db_1.query)(`SELECT id::text AS id, name, phone, city, 'listing' AS vendor_type
     ...
     LIMIT $4`, [`%${vendorType}%`, `%${stem}%`, `%${city}%`, limit]);
    return rows;
}
// ── GET /v1/public/wa-status — WhatsApp diagnostic ───────────
exports.workflowRouter.get('/wa-status', (_req, res) => {
...
exports.workflowRouter.post('/bookings', async (req, res) => {
    try {
        const { vendor_type, city, date_start, date_end, budget, seeker_name, seeker_phone, seeker_email, description, title, } = req.body;
        ...
        const [wfRow] = await (0, db_1.query)(`INSERT INTO workflow_requests
       ...
       RETURNING id`, [
            ...
            (0, whatsapp_1.normalizeWhatsAppPhone)(seeker_phone),
        ...
        for (const v of vendors) {
            const [match] = await (0, db_1.query)(`INSERT INTO workflow_vendor_matches
           (request_id, vendor_type, vendor_id, vendor_name, vendor_phone, vendor_city)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING id`, [requestId, v.vendor_type, v.id, v.name, v.phone, v.city]);
            if (v.phone) {
        ...
        await (0, db_1.query)(`UPDATE workflow_requests SET matched_count=$1, status=$2, updated_at=NOW() WHERE id=$3`, [vendors.length, vendors.length > 0 ? 'matching' : 'no_vendors', requestId]);
        // Notify seeker via WhatsApp + in-app notification
        const seekerPhone = (0, whatsapp_1.normalizeWhatsAppPhone)(seeker_phone);
        ...
        await (0, whatsapp_1.sendWhatsAppText)(seekerPhone, seekerMsg);
...
exports.workflowRouter.get('/bookings/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const wf = await (0, db_1.queryOne)(`SELECT id, type, title, description, city, vendor_type, date_start, date_end,
              budget, seeker_name, seeker_phone, seeker_email, status, matched_count, notes, created_at
       FROM workflow_requests WHERE id = $1`, [id]);
        if (!wf)
            return fail(res, 'Booking not found', 404);
        const matches = await (0, db_1.query)(`SELECT id, vendor_name, vendor_city, vendor_type, status, notes, quote_amount, responded_at, wa_notified_at
       FROM workflow_vendor_matches
       WHERE request_id = $1
       ORDER BY status DESC, created_at`, [id]);
...
exports.workflowRouter.get('/bookings', async (req, res) => {
    try {
        const phone = String(req.query.phone || '').replace(/\D/g, '');
        if (!phone)
            return fail(res, 'phone is required');
        const rows = await (0, db_1.query)(`SELECT id, type, title, city, vendor_type, date_start, date_end,
...
exports.workflowRouter.get('/notifications', async (req, res) => {
    try {
        const phone = String(req.query.phone || '').replace(/\D/g, '');
        if (!phone)
            return fail(res, 'phone is required');
        const rows = await (0, db_1.query)(`SELECT id, type, title, body, action_type, action_data, ref_type, ref_id, is_read, created_at
       FROM notifications
       WHERE phone = $1
       ORDER BY created_at DESC
       LIMIT 50`, [phone]);
...
exports.workflowRouter.patch('/notifications/:id/read', async (req, res) => {
    try {
        const { id } = req.params;
...
exports.workflowRouter.patch('/notifications/read-all', async (req, res) => {
    try {
        const phone = String(req.body.phone || '').replace(/\D/g, '');
        if (!phone)
            return fail(res, 'phone is required');
        await (0, db_1.query)(`UPDATE notifications SET is_read = TRUE, read_at = NOW() WHERE phone = $1 AND is_read = FALSE`, [phone]);
...
async function handleVendorWorkflowReply(vendorPhone, text) {
    ...
    if (!isYes && !isNo && !commentMatch)
        return false;
    // Find the most recent pending/notified match for this vendor phone
    const match = await (0, db_1.queryOne)(`SELECT wm.*, wr.title, wr.vendor_type, wr.city, wr.date_start, wr.date_end,
    ...
    if (isNo) {
        await (0, db_1.query)(`UPDATE workflow_vendor_matches SET status='rejected', responded_at=NOW() WHERE id=$1`, [match.id]);
        await (0, whatsapp_1.sendWhatsAppText)(vendorPhone, `✅ *Got it.* You've declined this request. No further action needed.\n\nIf you change your mind, reply *YES* before the booking date.`);
        const seekerPhone = match.seeker_phone;
        if (seekerPhone) {
            await createNotification({
                phone: seekerPhone,
    ...
    if (commentMatch) {
       ...
       WHERE id=$3`, [note, quoteAmt, match.id]);
        await (0, whatsapp_1.sendWhatsAppText)(vendorPhone, `✅ *Message received.* The seeker has been notified of your message.`);
        const seekerPhone = match.seeker_phone;
        if (seekerPhone) {
            const title = isQuote && quoteAmt
                ? `${vendorName} quoted ₹${quoteAmt.toLocaleString('en-IN')}`
                : `${vendorName} sent a message`;
            await (0, whatsapp_1.sendWhatsAppText)(seekerPhone, `💬 *Message from ${vendorName}:*\n\n"${note}"\n\nReply in the app to continue the conversation:\nhttps://demandgenius.vercel.app/explore`);
            await createNotification({
                phone: seekerPhone,
                type: 'vendor_message',
                title,
