...
async function query(sql, params = []) {
    ...
    try {
        ...
        logger_1.logger.debug('📊 DB Query', {
            sql,
...
async function queryWithTenant(tenantId, sql, params = []) {
    ...
    try {
        ...
        logger_1.logger.error('❌ Tenant query failed', { error: err.message });
        throw err;
    }
    finally {
        client.release();
...
async function withTransaction(fn) {
    ...
    try {
        ...
        await client.query('ROLLBACK');
        logger_1.logger.error('❌ Transaction failed', { error: err.message });
        throw err;
    }
    finally {
        client.release();
...
async function waitForDb(retries = 5, delayMs = 3000) {
    for (let i = 1; i <= retries; i++) {
        ...
        logger_1.logger.warn(`⏳ DB retry ${i}/${retries}...`);
...
async function closeDb() {
    try {
        ...
        logger_1.logger.error('❌ Error closing DB', { error: err.message });
    }
}
async function runMigrations() {
    ...
    `);
        logger_1.logger.info('Running migrations for dplanning database...');
        const migrations = getMigrations();
        for (const m of migrations) {
            const exists = await client.query('SELECT id FROM _migrations WHERE name = $1', [m.name]);
            if (exists.rows.length === 0) {
                ...
                try {
                    ...
                    await client.query('COMMIT');
                    logger_1.logger.info(`Migration complete: ${m.name}`);
                }
                catch (migErr) {
                    await client.query('ROLLBACK').catch(() => { });
                    logger_1.logger.error(`Migration FAILED: ${m.name} — ${migErr.message}`);
                    throw migErr;
                }
            }
        }
        logger_1.logger.info('All migrations complete.');
    }
    finally {
        client.release();
    }
