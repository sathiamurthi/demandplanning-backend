...
export async function sendMail(options: MailOptions): Promise<any> {
  const from = `"${process.env.SMTP_FROM_NAME || 'DemandPlanning'}" <${process.env.SMTP_FROM_EMAIL || process.env.SMTP_USER || 'noreply@example.com'}>`;
...
export async function sendPasswordResetEmail(email: string, resetToken: string) {
  ...
  return sendMail({
    to: email,
    subject: 'Reset your DemandPlanning password',
    html: `
      <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;background:#0d0f14;color:#e2e8f0;border-radius:12px;">
        ...
        <p style="color:#475569;font-size:12px;">DemandPlanning · paariwalaconnect@gmail.com</p>
      </div>
    `,
  });
}
...
export async function sendWelcomeEmail(email: string, firstName: string, companyName: string) {
  ...
  return sendMail({
    to: email,
    subject: `Welcome to DemandPlanning, ${firstName || companyName}!`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;background:#0d0f14;color:#e2e8f0;border-radius:12px;">
        <h2 style="color:#34d399;margin-top:0;">Welcome aboard 🎉</h2>
        <p>Hi <strong>${firstName || 'there'}</strong>,</p>
        <p>Your <strong>${companyName}</strong> account on DemandPlanning is ready.</p>
