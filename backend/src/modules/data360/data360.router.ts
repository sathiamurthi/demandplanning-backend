...
function applyMapping(row: Record<string, any>, mapping: Record<string, string>): Record<string, any> {
  ...
  for (const [sourceField, value] of Object.entries(row)) {
    ...
    if (sourceField in mapping && !mapping[sourceField]?.trim()) continue;
...
data360Router.post('/batches/:id/generate', data360Auth, async (req: D360Req, res) => {
  try {
    ...
    const fieldNames: string[] = mappedRows[0] ? Object.keys(mappedRows[0].mappedFields) : (template.extraction_fields?.length ? template.extraction_fields : batch.extraction_fields);
    const documents: any[] = [];
    let status = 'ready';
    let errorMsg = '';
