...
// ── Setup: College ────────────────────────────────────────────
c360Router.post('/setup/college', c360Auth, async (req: C360Req, res) => {
  try {
    const { name, city, state, website, description, established_year, college_type,
            accreditation, programs, ranking, placement_stats } = req.body;
    if (!name?.trim()) { fail(res, 'College name is required'); return; }
    const [row] = await query<any>(
      `INSERT INTO c360_colleges
       ...
       RETURNING *`,
      [req.c360User.sub, name.trim(), city||null, state||null, website||null,
       description||null, established_year||null, college_type||null,
       accreditation||null, programs||[], ranking||null, placement_stats||null]
    );
    await query(`UPDATE c360_users SET setup_done=true WHERE id=$1`, [req.c360User.sub]);
    ok(res, row, 201);
  } catch (e: any) { fail(res, e.message, 500); }
});

c360Router.get('/setup/college', c360Auth, async (req: C360Req, res) => {
  try {
    const row = await queryOne<any>('SELECT * FROM c360_colleges WHERE created_by=$1 ORDER BY created_at DESC LIMIT 1', [req.c360User.sub]);
    ok(res, row || null);
  } catch (e: any) { fail(res, e.message, 500); }
});

// ── Setup: Expert Profile ─────────────────────────────────────
c360Router.post('/setup/expert', c360Auth, async (req: C360Req, res) => {
  try {
    const { designation, company, industry, linkedin, bio, skills, expertise_areas, years_experience } = req.body;
    const [row] = await query<any>(
      `INSERT INTO c360_expert_profiles
       ...
       RETURNING *`,
      [req.c360User.sub, designation||null, company||null, industry||null, linkedin||null,
...
c360Router.get('/setup/expert', c360Auth, async (req: C360Req, res) => {
  try {
    const row = await queryOne<any>('SELECT * FROM c360_expert_profiles WHERE user_id=$1', [req.c360User.sub]);
    ok(res, row || null);
  } catch (e: any) { fail(res, e.message, 500); }
});

// ── Setup: Training Center ────────────────────────────────────
c360Router.post('/setup/training-center', c360Auth, async (req: C360Req, res) => {
  try {
    const { name, city, state, website, description, courses, certifications, fee_range, placement_support } = req.body;
    if (!name?.trim()) { fail(res, 'Training center name is required'); return; }
    const [row] = await query<any>(
      `INSERT INTO c360_training_centers
...
c360Router.get('/setup/training-center', c360Auth, async (req: C360Req, res) => {
  try {
    const row = await queryOne<any>('SELECT * FROM c360_training_centers WHERE user_id=$1 ORDER BY created_at DESC LIMIT 1', [req.c360User.sub]);
    ok(res, row || null);
  } catch (e: any) { fail(res, e.message, 500); }
});

// ── Directory: unified search ─────────────────────────────────
c360Router.get('/directory', async (req, res) => {
  try {
    const q = (req.query.q as string || '').trim();
    const type = (req.query.type as string || 'all');

    const colleges = type === 'all' || type === 'college'
      ? await query<any>(`SELECT 'college' as entity_type, id, name, city, state, description, programs, accreditation, ranking, website, i360_verified, created_at FROM c360_colleges ${q ? "WHERE name ILIKE $1 OR city ILIKE $1" : ""} ORDER BY i360_verified DESC, created_at DESC LIMIT 20`, q ? [`%${q}%`] : [])
      : [];

    const experts = type === 'all' || type === 'expert'
      ? await query<any>(`SELECT 'expert' as entity_type, ep.id, u.name, ep.designation, ep.company, ep.industry, ep.bio, ep.skills, ep.expertise_areas, ep.years_experience, ep.linkedin, ep.i360_verified, ep.created_at FROM c360_expert_profiles ep JOIN c360_users u ON u.id=ep.user_id ${q ? "WHERE u.name ILIKE $1 OR ep.company ILIKE $1 OR ep.industry ILIKE $1 OR ep.designation ILIKE $1" : ""} ORDER BY ep.i360_verified DESC, ep.created_at DESC LIMIT 20`, q ? [`%${q}%`] : [])
      : [];

    const tcs = type === 'all' || type === 'training_center'
      ? await query<any>(`SELECT 'training_center' as entity_type, id, name, city, state, description, courses, certifications, fee_range, placement_support, website, i360_verified, created_at FROM c360_training_centers ${q ? "WHERE name ILIKE $1 OR city ILIKE $1" : ""} ORDER BY i360_verified DESC, created_at DESC LIMIT 20`, q ? [`%${q}%`] : [])
      : [];
...
// ── Admin: verify entry ────────────────────────────────────────
c360Router.patch('/admin/verify/:type/:id', async (req, res) => {
  if (req.headers['x-admin-key'] !== (process.env.ADMIN_SECRET || 'c360-admin')) {
    res.status(403).json({ success: false, error: 'Forbidden' }); return;
  }
  try {
...
c360Router.get('/admin/directory', async (req, res) => {
  if (req.headers['x-admin-key'] !== (process.env.ADMIN_SECRET || 'c360-admin')) {
    res.status(403).json({ success: false, error: 'Forbidden' }); return;
  }
  try {
    const colleges = await query<any>(`SELECT 'college' as entity_type, id, name, city, state, i360_verified, created_at FROM c360_colleges ORDER BY i360_verified ASC, created_at DESC`, []);
    const experts  = await query<any>(`SELECT 'expert' as entity_type, ep.id, u.name, ep.company, ep.designation, ep.i360_verified, ep.created_at FROM c360_expert_profiles ep JOIN c360_users u ON u.id=ep.user_id ORDER BY ep.i360_verified ASC, ep.created_at DESC`, []);
    const tcs      = await query<any>(`SELECT 'training_center' as entity_type, id, name, city, state, i360_verified, created_at FROM c360_training_centers ORDER BY i360_verified ASC, created_at DESC`, []);
    ok(res, { colleges, experts, training_centers: tcs });
  } catch (e: any) { fail(res, e.message, 500); }
});
